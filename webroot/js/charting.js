$(document).ready(function () {
	...
	$("#site").val($('input[type="checkbox"][name="sites\\[\\]"]:checked:first').val());
    });
	...
	//update chart label
	$('#chartTitle').html('<strong style="font-size:14pt;">' + $('#measurementSelect option:selected').text() + " Histogram</strong>");
	//remove the old chart & regenerate the histogram
	$("svg").empty();
	getGraphData($('#startdate').val(), $('#enddate').val(), $('#measurementSelect').val(), "bar");
    });
    //Line Graph stuff
	...
	var comData = [];
	for (var q = 0; q < sites.length; q++) {
	    //**Begin Pre-parsing data by site */
		...
		if (numPoints !== 0) {
			...
			var obj = {date: currentDatePoint.toISOString(), value: "0", site: sites[q]};
	...
	//it is important that the min/max lines are last to be added to the SVG, this ensures
	//that they are above the bars in the histogram
	...
	if (benchmark.min !== null) {
		...
		.data([
		    {y1: y(benchmark.min), y2: y(benchmark.min), x1: 0, x2: width, className: "minLineBackground", dashArray: "8,5", color: "white", opacity: 1, lineWidth: 4},
		    {y1: y(benchmark.min), y2: y(benchmark.min), x1: 0, x2: width, className: "minLine", dashArray: "8,5", color: "purple", opacity: 0.8, lineWidth: 1.5}
		])
		.enter().append("line")
	...
	if (benchmark.max !== null) {
		...
		.data([
		    {y1: y(benchmark.max), y2: y(benchmark.max), x1: 0, x2: width, className: "maxLineBackground", dashArray: "8,5", color: "white", opacity: 1, lineWidth: 4},
		    {y1: y(benchmark.max), y2: y(benchmark.max), x1: 0, x2: width, className: "maxLine", dashArray: "8,5", color: "red", opacity: 0.8, lineWidth: 1.5}
		])
		.enter().append("line")
		.attr("class", function (d) {
		    return d.c1lassName;
	...
	let boxColor = "#DDDDDD";
	let boxData = [
	    {"x": 0 - margin.left, "y": 0, "width": margin.left, "height": objectHeight, "fill": boxColor, "opacity": 1},
	    {"x": objectWidth, "y": 0, "width": margin.right + 30, "height": objectHeight, "fill": boxColor, "opacity": 1}
	];
