...
<div class = "container roundGreyBox">
    ...
    <table id='tableView'  class="table table-striped table-responsive">
		...
		foreach ($bacteriaSamples as $bacteriaSample):
			...
			<td><?= $this->Number->format($bacteriaSample->measure) ?></td>
...
<script>
	...
	form.setAttribute("action", "<?= $this->Html->Url->build(['controller' => 'BacteriaSamples', 'action' => 'chartview']); ?>");
